...
import ActivarInteresadoPage from './pages/ActivarInteresadoPage';
import './styles.css';
...
//   ?activarToken={token} → "terminá tu alta" de un interesado, emitido por InteresadosService.invitarTodos()
...
const esActivarInteresado = qs.has('activarToken');

function Root() {
  if (esAdmin) return <AdminPage />;
  if (esResetPassword) return <ResetPasswordPage />;
  if (esActivarInteresado) return <ActivarInteresadoPage />;
