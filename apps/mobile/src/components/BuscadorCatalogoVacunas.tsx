 ...
 * antiparasitarios comunes) filtrado por especie — mismo criterio que la
 * web (`BuscadorCatalogoVacunas.tsx`): el texto siempre es libre (la guía es
 * chica, no cubre todo), filtrado client-side. Los resultados se listan
 * abajo del campo (no en un dropdown flotante), mismo patrón que el resto
 * de los buscadores del mobile (`SeleccionarAnimalModal`, `SelectorDueno`).
 */
export function BuscadorCatalogoVacunas({
  ...
  return (
    <View>
      <Field label="Producto" value={valor} onChangeText={onCambiar} placeholder={placeholder} />
      {resultados.map((c) => (
