...
- [ ] **4.1 — Alta de vacuna:** bajar a "Vacunaciones" → "+ Nueva vacuna" → cargar producto (ej. "Rabia"), fecha de aplicación, próxima dosis (poné una fecha dentro de los próximos 30 días para el paso siguiente), lote opcional, costo (obligatorio, 2026-09-06 — probar también dejarlo en 0). Guardar. Debe aparecer en la tabla.
...
- [ ] **15.5 — Ficha: vacunación con calendario nativo:** en "Nueva vacunación", el campo "Próxima dosis" debe abrir un selector de fecha nativo (no un teclado para tipear texto) al tocarlo. Elegir una fecha → debe verse formateada en el campo. Cargar también un costo (2026-09-06). Guardar y confirmar que se ve en el historial, costo incluido.
