...
| Deploy en VPS (Postgres real) | ⏳ | Hoy corre local con PGlite. La migración (`drizzle-kit migrate`) y el seed (`db:seed`, driver-agnóstico) ya se probaron contra un Postgres real local (2026-08-28) y quedaron corregidos (ver `CHANGELOG.md`). El plan de despliegue completo ya está escrito en `docs/Plan_Despliegue.md` (2026-09-03, arquitectura revisada a Docker el 2026-09-05 — un contenedor por servicio, un Caddy "edge" compartido entre proyectos vía `caddy-docker-proxy`, Postgres propio por proyecto) — sólo falta contratar y aprovisionar la VPS en sí (F0.1) y ejecutarlo. Auditoría de production-readiness hecha (2026-09-05): rate-limiting en auth, dominio de Resend, `GET /health`, error tracking y build de mobile son los gaps concretos a cerrar antes de invitar usuarios reales, todos anotados en el checklist del plan. |

**Verticales cerradas:** el "reality check" (backend real por HTTP) y "hacerlo tangible" (web funcional) están completos. El ecosistema funciona hoy de punta a punta: navegador → API → base, con el turnero operando desde el mostrador. La HC clínica ya cierra el circuito completo — consultas (con edición/borrado), vacunaciones, recordatorios, carnet y portal — así que el **MVP funcional de historia clínica está cerrado**, y la sesión tiene refresh token. Tropera tiene establecimientos + hacienda por categoría + movimientos con historial y validación de stock + eventos sanitarios/reproductivos + panel consolidado + offline (F1.1–F1.6) — el backend de offline está probado (`test:sync-demo`), y desde el 2026-09-04 el mobile corre de punta a punta en un dispositivo físico real, no sólo el backend. Farmacia tiene su MVP básico (catálogo + stock + movimientos, F4.2 parcial) más la dispensa ligada a consulta (F4.3). Migración y seed ya se validaron contra un Postgres real. Analítica de uso, catálogos de referencia, landing page pública y recuperación de contraseña están cerrados de punta a punta (2026-09-04/05). Lo que queda pendiente (búsqueda de animales en servidor, un build de release del mobile, HCE offline, facturación, y ejecutar el deploy en VPS) es robustez, alcance y ejecución, no funcionalidad core faltante en lo ya construido.
...
|---|---|---|
| **Infraestructura (prod)** | **VPS paga** (ver `docs/Plan_Despliegue.md` para el detalle de recursos/proveedor) | ⏳ Pendiente de contratar/aprovisionar (F0.1). Se descartó la idea original de una VM de Oracle Cloud Always Free (ARM Ampere) — capa gratuita atractiva en el papel, pero con recursos ARM más chicos e imprevisibles para un primer deploy; una VPS paga chica (2 vCPU/4GB) sale unos pocos dólares al mes y da un entorno x86 estándar, sin las particularidades de Always Free. |
...
| **App móvil** | **React Native + Expo** | 🟡 Login + Tropera + HCE (Home, ficha, consultas/vacunaciones, Turnos, Farmacia) conectados a `sync/`, verificado en un dispositivo físico real desde 2026-09-04; falta un build de release firmado (EAS Build) para instalar fuera del dev-client de la PC. |
...
Con el core + HC (consultas + vacunaciones + recordatorios + carnet) + turnero + portal del dueño + admin (incluida la facturación de plataforma y el onboarding, Fase 5c) + refresh token + Tropera completo (F1.1–F1.6, incluido offline verificado en un dispositivo físico real) + Farmacia (catálogo + stock + movimientos + dispensa ligada a consulta, F4.3) + Huella operativo (atender, caja, costo, estadísticas) + mobile con HCE completo + analítica de uso + catálogos de referencia + landing page + recuperación de contraseña funcionando de punta a punta — el foco pasa a **ejecutar el deploy en VPS** primero, y después a decidir la profundidad de cada vertical restante. Notificaciones proactivas (email/WhatsApp automático) quedan en pausa por decisión explícita, no por orden de prioridad.

De mayor a menor valor:

1. **Producción:** contratar y aprovisionar la VPS (F0.1) y desplegar siguiendo `docs/Plan_Despliegue.md` (revisado 2026-09-05, arquitectura Docker) — arquitectura, recursos recomendados, variables de entorno y checklist paso a paso ya están escritos, sólo falta ejecutarlo. Antes de invitar usuarios reales, cerrar los gaps de la auditoría de production-readiness (2026-09-05, ver el checklist del plan): verificar un dominio propio en Resend (hoy el mail de recuperación de contraseña no le llega a nadie más que a la cuenta de Resend), sumar rate-limiting a los endpoints de auth, un `GET /health`, y decidir si el testeo real con usuarios incluye mobile (en cuyo caso hace falta EAS Build antes, ver más abajo) o arranca sólo por web.
2. **Build de release para mobile** (EAS Build o `assembleRelease` con keystore propio, y sacar el `package` del placeholder `com.anonymous.mobile` en `app.json`) — sólo necesario si el testeo con usuarios reales va a incluir instalar la app en celulares de terceros, no en el propio dispositivo de desarrollo.
3. Facturación (Fase 5, ARCA/WSFE): reservada por la complejidad regulatoria, no se tocó ni se va a tocar sin una conversación aparte. No confundir con el "Home de facturación" ya cerrado en la Fase 5c (control de pagos manual entre la plataforma y sus organizaciones clientes) — son dos cosas distintas.
