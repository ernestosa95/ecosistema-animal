...
| Carnet PDF del paciente | ✅ | Descargable desde la ficha (`GET /animales/:id/carnet.pdf`). Ya consulta datos reales (animal, especie, dueño, vacunaciones) vía Drizzle — dejó de ser mock. Pendiente menor: `personas` no modela domicilio, así que el carnet lo imprime como `—`. |
...
## 🧭 Fase 6 previa — Evolución UI/UX (spec grande, 2026-08-28) — Fase A ✅ · Fase B 🟡 · Fase C 🟡 · Fase D 🟡
...
**Entregable:** backend probado por API contra un backend efímero (Caja tampoco tiene suite `test:*-demo` propia todavía, igual que Tropera): apertura/rechazo de doble apertura, cobro sin caja abierta rechazado, venta con descuento real de stock, egreso, cierre con diferencia calculada correctamente (781 vs. 780 declarado → `pendiente`), auditoría con validación de observación obligatoria, honorarios y liquidación funcionando. `nest build`/`vite build` limpios. **Sin verificar en el navegador.** Sin arrancar: Fase E (campo individual, bloqueada por la decisión de Tropera sin tomar).
