...
**El hallazgo**: varios endpoints de la Fase E (`evaluaciones-andrologicas`, `protocolos-iatf`, y también `muestras`/`toros-virtuales`/`hallazgos`/`tareas`) aceptan el rol `veterinario` — tiene sentido, diagnóstico reproductivo y evaluación andrológica son trabajo de un veterinario sanitarista. Pero `ROLES_TROPERA` en `nav/config.ts` era sólo `propietario/admin/capataz`: un usuario con únicamente el rol `veterinario` no podía ver la pestaña "Tropera" en absoluto, así que nunca llegaba a esas pantallas. Al investigar si simplemente agregar `veterinario` al set alcanzaba, encontré que **no** — `TroperaPage.tsx` (2600+ líneas) no tenía ningún control de rol interno, todo el módulo se mostraba u ocultaba en bloque según ese único set. El backend en realidad usa **tres combinaciones distintas** de roles según la sub-feature (sólo capataz / sólo veterinario / ambos), así que sumar `veterinario` sin más iba a reproducir el mismo bug que se acababa de corregir en la ficha del paciente, en un lugar nuevo — y de paso reveló que **el bug ya existía hoy** para `capataz` en las dos sub-features veterinario-only (un capataz ya veía botones de evaluación andrológica/protocolos IATF que el backend le iba a rechazar).

- `nav/config.ts`: `ROLES_TROPERA` ahora incluye `veterinario` (visibilidad de la pestaña). Dos sets nuevos para el gating fino, calcados de lo que exige el backend en cada sub-feature: `ROLES_TROPERA_CAMPO` (propietario/admin/capataz — establecimientos, potreros, existencias, movimientos, fichas individuales, plantillas de tareas) y `ROLES_TROPERA_VETERINARIO` (propietario/admin/veterinario — evaluación andrológica, protocolos IATF).
- `TroperaPage.tsx`: cada botón/formulario de escritura de esas dos categorías ahora se oculta según corresponda (alta/edición de establecimiento, potreros, corrección de existencias, movimientos, alta/edición/conciliación de animales individuales, plantillas de tareas — todos `ROLES_TROPERA_CAMPO`; evaluación andrológica y protocolos IATF — `ROLES_TROPERA_VETERINARIO`). Las acciones que el backend ya permite a los 4 roles por igual (eventos, muestras, toros virtuales, hallazgos, marcar una tarea completada/cancelada) no necesitaron gating adicional — ya alcanzaba con la visibilidad de la pestaña. La lectura (listados, fichas) sigue abierta a todos los roles que entran a Tropera, igual que el resto de la app.

**Verificado**: `vite build` limpio en web. `test:tropera-demo` sigue en 41/41 (no se tocó nada del backend, este era un gap puramente de exposición del frontend). Mobile no tiene Tropera Fase E todavía, no aplica.

## [2026-09-05] — Fix: la ficha del paciente no ocultaba las acciones clínicas para roles sin permiso

El usuario preguntó por qué el Home oculta "Nueva consulta"/"Registro de vacuna" para roles administrativos pero quiso confirmar que la misma regla se respeta en todos lados. No era así: `HuellaHomeSection.tsx` (Home) ya gateaba esos accesos rápidos con `ROLES_CLINICO` (propietario/admin/veterinario, mismo set que exige el backend en `consultas`/`vacunaciones`/`indicaciones`/`farmacia/movimientos`), pero `PacienteDetallePage.tsx` — a la que también se llega desde "Animales" o "Turnos", ambas abiertas para `recepcion` — no tenía ningún control de rol: mostraba "+ Nueva consulta", "+ Nueva" (vacuna) y el menú "⋮" de cada consulta (Dispensar/Indicación/Editar/Borrar) a cualquiera, y recién el backend cortaba con un 403 al guardar. No era un agujero de seguridad (el backend ya validaba bien), pero sí una UI que prometía algo que no iba a andar.

- Web: `PacienteDetallePage.tsx` suma `puedeClinico = tieneAlguno(sesion.roles, ROLES_CLINICO)` y lo usa para ocultar el botón/form de "+ Nueva consulta", el botón/drawer de "+ Nueva" vacuna, y el botón "⋮" completo (todas las acciones que cuelgan de él son del mismo set de roles en el backend, así que un solo flag alcanza).
- Mobile: mismo fix en `app/paciente/[id].tsx`, usando el `ROLES_CLINICO` ya existente en `nav/roles.ts` (portado ahí desde que se armó el Home de mobile).
- La lectura de la historia clínica (tablas, línea de tiempo) sigue visible para todos los roles — el backend tampoco restringe esas lecturas, y a recepción le sirve verla igual.

**Verificado**: `vite build` limpio en web, `tsc --noEmit` limpio en mobile. No hizo falta tocar el backend, los guards ahí ya estaban bien.

## [2026-09-05] — Fix: la web dejaba crear un animal sin dueño

El usuario detectó, probando el modal "Nuevo turno — elegir paciente", que se podía dar de alta un paciente sin asignarle un dueño ("Sin dueño asignado" como opción del `<select>`). Es una regla de negocio básica de Huella — todo animal identificado tiene que estar asociado a un dueño — y sólo el flujo viejo de la web la venía violando (mobile ya la tenía bien resuelta, ver más abajo).

- Backend: `CreateAnimalDto.personaId` pasa de opcional a **requerido** (`@IsUUID()` sin `@IsOptional()`). `UpdateAnimalDto.personaId` no se toca a "opcional" sino a "no se puede vaciar" — usa `@ValidateIf((_o, value) => value !== undefined)` en vez de `@IsOptional()`, porque esta última salta la validación tanto con el campo ausente como con `null`, y acá el caso a bloquear es justo un `personaId: null` explícito (dejaría a un paciente ya identificado sin dueño).
- Web, tres lugares donde se podía crear un animal sin elegir dueño, los tres con el mismo arreglo (sacar la opción "Sin dueño asignado", `<select required>`, guarda en JS por si acaso): `PacientesPage.tsx` (alta principal), `components/SeleccionarAnimalModal.tsx` (el modal del screenshot, reusado en los accesos rápidos del Home), `pages/TurnosPage.tsx` → `ModalNuevo` (alta de paciente embebida en "Nuevo turno"). Se verificó además que `api/turnos.ts` → `crearPacienteRapido()` (llamada por este último) ya resolvía el `personaId` antes de pegarle a `POST /animales` en todos los casos — no necesitó cambios.
- Mobile: revisado, no tenía el bug. `SelectorDueno.tsx`/`altaPaciente.ts` ya exigían dueño desde que se construyó (`components/SelectorDueno.tsx` lo documenta explícitamente: "a diferencia de la web, que sí lo permite"). Con este fix esa diferencia entre plataformas desaparece.
- **Dato encontrado, sin resolver todavía**: en el `pgdata` real de desarrollo hay 2 animales ya cargados sin dueño (creados antes de este fix, cuando el hueco todavía existía) — pendiente de decidir con el usuario qué hacer con esos dos casos puntuales (no se tocaron).

**Verificado**: `tsc --noEmit` limpio en backend, `vite build` limpio en web. Las 18 `test:*-demo` pasan (0 fallas — `animales-flow.demo.ts` llama al service directamente sin pasar por el DTO, así que no iba a fallar por este cambio, pero se corrió igual por las dudas). Backend reintegrado vía el respawn automático de `nest start --watch` tras el cambio de los DTOs. **Sin probar clic a clic en el navegador** — pendiente.

## [2026-09-05] — Logo propio de la organización (carnet, ficha, portal del dueño)

A pedido del usuario: el propietario/admin puede cargar el logo de su veterinaria/campo, y ese logo tiene que verse en todo lo que llega al dueño de la mascota — no sólo la marca "Huella" de la plataforma.

- `core.organizaciones` suma `logo_url` (nullable — migración `0033_useful_agent_brand.sql`, aplicada contra el `pgdata` real con el procedimiento de siempre, conteos iguales antes/después).
- Nuevo `POST /organizacion/logo` (self-service, propietario/admin, multipart — mismo patrón multer/diskStorage que la foto de animal y el comprobante de pago, `common/logo-organizacion-upload.ts`). Sin SVG a propósito: el mismo archivo se embebe tal cual tanto en un `<img>` del portal como en el `Image` de `@react-pdf/renderer` del carnet/ficha, que sólo soporta raster — un SVG se vería bien en un lado y roto en el otro. `GET /organizacion/plan` ahora también devuelve `nombre`/`logoUrl` de la organización.
- Web, "Mi plan": nueva card "Logo de tu organización" (preview + subir/reemplazar) arriba de los datos de plan/facturación.
- **Carnet y ficha PDF** (`hce/carnet/`): el encabezado (antes fijo "🐾 Huella") ahora muestra el logo + nombre de la organización si cargó uno, con un fondo blanco detrás (el header es de color, y un logo con fondo transparente pensado para blanco se vería mal directamente sobre él) — sin logo propio, sigue mostrando la marca de Huella como siempre. `CarnetService.buildData()` resuelve el logo con un query nuevo a `organizaciones`.
- **Portal del dueño**, las dos superficies (`hce/portal/` por código y `portal/` por magic-link): mismo criterio en el header — logo + nombre si hay, si no "🐾 Huella". Ambos `PortalService.resumen()`/`resumenPorCodigo()` ahora resuelven y devuelven `organizacion: { nombre, logoUrl }`.
- Bug preexistente evitado antes de que apareciera: se agregó `logo_url text` al DDL inline de `core.organizaciones` en los 18 `test:*-demo` **en el mismo commit** que se agregó la columna real — la lección de `password_changed_at` (dos entradas atrás) fue no repetir el desfasaje.

**Verificado**: `tsc --noEmit` limpio en backend y web. Las 18 `test:*-demo` pasan (0 fallas — ninguna cubre `CarnetService`/las dos `PortalService` directamente, así que esto no sumó casos nuevos, pero tampoco rompió nada existente). Migración `0033` aplicada contra el `pgdata` real, backend reintegrado y confirmado sirviendo `POST /organizacion/logo` (401 sin token, no 404). **Sin probar clic a clic en el navegador ni mirar un PDF generado con logo real** — pendiente.

## [2026-09-05] — Verificación de email por código, recordatorio de pago, revisión de seguridad del lote anterior

Continuación de la sesión de más abajo, a partir de tres pedidos del usuario pensando en producción: preparar el envío de mail para un dominio propio, verificación de email al crear la cuenta, y recordatorio de pago.

### Verificación de email por código, antes de mandar la solicitud
- Nuevos endpoints públicos en `solicitudes/`: `POST /solicitudes/verificar-email` (manda un código de 6 dígitos) y `POST /solicitudes/verificar-email/confirmar`. Stateless, mismo criterio que el reset de contraseña — el código viaja embebido en un JWT de 10 minutos que el cliente retiene (no hay tabla nueva); al confirmar, se emite un segundo JWT (`scope: 'email_verificado'`, 30 minutos) que hay que adjuntar a `POST /solicitudes` — `CrearSolicitudDto` ahora exige `emailVerificadoToken` y `SolicitudesService.crear()` valida que el email embebido coincida con el del formulario. Throttling propio (5 cada 15 min por IP, mismo criterio que `auth.controller.ts`).
- Web: paso "Tu acceso" del wizard de alta suma el flujo completo — "Enviar código" → input de 6 dígitos → "Confirmar código" → chip "✓ Email verificado" (con "Cambiar email" para reiniciar si hace falta). El email se bloquea (`readOnly`) una vez verificado; cambiarlo invalida la verificación. "Enviar solicitud" rechaza sin verificar, tanto del lado del cliente como del servidor.

### Recordatorio de pago por email (manual, con la puerta abierta a automatizarlo después)
- El usuario pidió automático pero con posibilidad de disparo manual desde admin — se construyó la versión manual primero (no hay ningún scheduler en el backend todavía) pero con `AdminService.enviarRecordatorioPago()` como un método de servicio aparte, para que un futuro job sólo tenga que llamarlo, sin duplicar la lógica del mail.
- Nuevo `POST /admin/organizaciones/:id/recordatorio-pago`: le manda un mail a los propietarios/admins activos de la organización con el próximo vencimiento. Botón "Enviar recordatorio" nuevo junto a "Registrar pago" en la tabla de Home.

### Revisión de código del lote anterior (`/code-review high`) — 10 hallazgos, la mayoría corregidos
El más serio: el nuevo self-service `PATCH /usuarios/:id/roles` (entrada anterior) dejaba que un `admin` (no propietario) se auto-otorgara el rol de `propietario` — sin nueva capacidad operativa directa (admin y propietario ya tienen el mismo nivel de acceso, ver `config/rolesInfo.ts`), pero sí bypaseaba la protección de `resetearPassword()` ("sólo un propietario puede resetear la contraseña de otro propietario"). Corregido: `actualizarRoles()` ahora exige que quien ejecuta la acción ya sea propietario para poder otorgarle ese rol a alguien (a sí mismo o a otro) — cubierto con 4 casos nuevos en `test:usuarios-demo` (18/18 ahora). Otros corregidos: `SelectorBusqueda` no confirmaba un match exacto tipeado si se perdía el foco sin clickear la sugerencia (afectaba el campo Provincia, ahora obligatorio); la ventana entre guardar el token de `/admin` en localStorage y confirmar que es super-admin (ahora se confirma antes de persistirlo, no después); el wizard obligatorio (entrada anterior) no tenía ninguna salida — se agregó un link "Cerrar sesión"; `sesion.roles` quedaba desactualizado en el resto de la app si el propio propietario se agregaba un rol en el wizard (nuevo `useSesion().actualizarRoles()`); dos clicks rápidos en el wizard podían pisarse por una condición de carrera (ahora los checkboxes del miembro se deshabilitan mientras guarda); "Pagos anteriores" del modal de `/admin` no distinguía un pago pendiente/rechazado de uno confirmado; un doble submit podía crear dos pagos `pendiente` para la misma organización (ahora el backend lo rechaza, no sólo la UI). **Sin corregir, documentado para después**: `InfoRoles`/las tarjetas de "este plan incluye" tratan un rol ausente de `limitesRoles` como "no incluido" en vez de "sin límite" — no aplica hoy porque todo plan se crea enumerando los 5 roles, pero conviene tenerlo presente si eso cambia; la protección de "no dejar la org sin propietario" está duplicada casi textual entre `AdminService` y `UsuariosService` (un futuro fix a una copia podría no llegar a la otra).

### Envío de mail para producción (dominio propio) — sin cambios de código
Relevado, no implementado — es un trámite del usuario en el panel de Resend + su proveedor de DNS, no algo que se resuelva con código (`MAIL_FROM`/`RESEND_API_KEY` ya se leen de env, no hace falta tocar nada del lado del backend una vez verificado). Instrucciones dadas en la conversación; el checklist ya existente en `docs/Plan_Despliegue.md` (sección 8, "Dominio propio verificado en Resend") sigue siendo la referencia. De paso, se detectó que **hoy, con `RESEND_API_KEY` configurada en el `.env` local**, el flujo de "olvidé mi contraseña" no cae en su fallback de loguear el link en consola (eso sólo pasa sin API key) — intenta un envío real que Resend en modo sandbox sólo entrega a la casilla dueña de la cuenta, así que probarlo hoy con cualquier otro email no muestra el link en ningún lado. Documentado, no es un bug — se resuelve solo en cuanto haya un dominio verificado.

**Verificado**: `tsc --noEmit` limpio en backend y web. Las 18 `test:*-demo` pasan (`usuarios-demo` sumó 4 casos nuevos para el fix de seguridad, `solicitudes-demo` sumó 5 para la verificación por código — 39 checks nuevos en total). Los endpoints nuevos confirmados en vivo contra el backend real (responden 401/400 según corresponde, no 404). **Sin probar clic a clic en el navegador** — todo lo de esta entrada quedó pendiente de una pasada visual, igual que el resto del lote del día.

## [2026-09-05] — Alta pública rediseñada, equipo/roles self-service, "Mi plan" y pago por transferencia

Sesión iterativa a partir de feedback del usuario probando el alta de una organización nueva de punta a punta.

### Alta pública (`LoginPage.tsx`, modo registro)
- Rediseño completo: de una sola columna vertical larga a un **wizard de 4 pasos** ("Tus datos" / "Tu organización" / "Tu plan" / "Tu acceso"), con barra de progreso, campos en `.form-grid` de a 2 columnas, y estilo alineado a la landing (`landing-serif`, `landing-btn` píldora verde) — antes usaba los mismos estilos genéricos que el login simple. Avanzar de paso reusa la validación nativa del `<form>` (el botón "Siguiente" es `type="submit"`; sólo los campos del paso visible están en el DOM).
- `Localidad` y `Provincia` pasan a obligatorias; `Provincia` ahora es un `SelectorBusqueda` (mismo componente reusable de Farmacia) sobre las 24 provincias argentinas (`config/provinciasArgentina.ts`) — texto libre a nivel DTO/DB, la lista cerrada es sólo restricción de UI. Backend: `localidadOrganizacion`/`provinciaOrganizacion` pasan de opcionales a requeridas en `CrearSolicitudDto`.
- `Tipo` de organización: el `<select>` ahora sólo ofrece "Clínica veterinaria o similares" (deshabilitado, sin otra opción) — Establecimiento/Mixta no tienen sentido en el alta pública hoy.
- El checkbox de términos y condiciones bloqueaba el botón "Enviar solicitud" vía `disabled` — al no aceptarlo, el click no hacía nada, sin ningún aviso. Se sacó esa condición del `disabled` (sólo depende de `cargando`) para que el intento de submit dispare la validación nativa del checkbox `required`, que sí muestra el aviso del browser.
- "¿Qué puede hacer cada rol?" (`InfoRoles`) suma un prop `soloRoles` — en el paso "Tu plan" filtra a sólo los roles que el plan elegido realmente incluye (cupo > 0), en vez de listar los 5 roles fijos siempre. En `/admin` (Planes) sigue mostrando todos, sin plan puntual en contexto.
- Landing y wizard: los cupos por rol de cada plan ahora filtran los que están en 0 (antes la landing mostraba "Hasta 0 administrador").

### Fix: un usuario normal podía "entrar" a `/admin`
- `POST /auth/login` no distingue super-admin de un usuario común, así que cualquier cuenta válida lograba loguearse en la pantalla de `/admin` y recién se enteraba de que no tenía permiso al ver el JSON crudo de un 403 en la primera card que cargaba. Nuevo `GET /admin/whoami` (mismo `SuperAdminGuard`) — el `login()` del cliente admin lo llama justo después de loguear; si falla, descarta el token y nunca deja entrar al panel.
- Relacionado: cualquier 401 dentro del panel (token vencido) ahora redirige solo a la pantalla de login con "Tu sesión expiró. Volvé a ingresar." — antes quedaba mostrando el panel roto con el JSON del error. `api/admin.ts` expone `suscribirseAExpiracion()`, que `AdminPage.tsx` usa para volver a `logueado: false` desde cualquier llamada fallida.

### Admin → Planes: editar nombre y descripción
- El botón "Editar límites" pasó a ser "Editar" y ahora también permite cambiar `nombre` y `descripcion` del plan (antes sólo se podían cargar al crearlo) — la descripción es justamente la que se muestra en la landing y en el paso "Tu plan" del alta. El backend ya soportaba `nombre`/`descripcion` en `ActualizarPlanDto`, sólo faltaba exponerlo en la UI.

### Wizard de configuración rápida: ya no se puede saltar + "Tu equipo" rediseñado
- Se sacaron todos los botones "Saltar por ahora" (bienvenida, equipo, agendas) — a pedido del negocio, el setup inicial pasa a ser obligatorio de principio a fin en vez de poder abandonarse con un click en cualquier paso.
- Paso "Tu equipo": en vez de arrancar directo con el formulario de alta (confuso para el primer usuario, cuyo único miembro es él mismo), ahora muestra primero el listado de miembros existentes con checkboxes por rol para asignarles roles adicionales in place (ej. el propietario también puede marcarse como veterinario). "+ Agregar usuario" queda al final, para cuando el rol que hace falta todavía no lo cubre nadie. Esto requirió un endpoint que no existía: `PATCH /usuarios/:id/roles` (self-service, propietario/admin de la propia organización — antes esa acción sólo existía para el super-admin vía `/admin`), con las mismas protecciones que su equivalente de super-admin (no deja sin propietario activo a la organización, respeta el cupo del plan).
- El paso "Peluquería canina (opcional)" se generalizó a **"Agenda no médica"**, con peluquería canina sólo como ejemplo en la descripción — el nombre de la agenda pasó de estar fijo en "Peluquería canina" a un campo de texto libre (placeholder `Ej: Peluquería canina`), así sirve para cualquier servicio sin veterinario de por medio.

### "Mi plan" + pago por transferencia con comprobante
- Nueva sección "Mi plan" en el menú desplegable del usuario (mismo lugar que "Usuarios", visible sólo a propietario/admin): recuerda plan actual, próximo vencimiento, si ya pagó este mes, y qué incluye el plan; formulario "Registrar pago" y el historial con estado (pendiente/confirmado/rechazado).
- Backend, módulo nuevo `core/organizacion/` (self-service, distinto de `admin/` que es del super-admin sobre cualquier organización): `GET /organizacion/plan`, `GET /organizacion/pagos`, `POST /organizacion/pagos` (multipart). Por ahora la única vía es transferencia — se sube el comprobante (imagen o PDF, `common/comprobante-pago-upload.ts`, mismo patrón multer/diskStorage que la foto de animal) y el pago nace `pendiente`, sin contar todavía como "pagó este mes".
- `plataforma.pagos` suma `estado` (`pendiente`/`confirmado`/`rechazado`, default `confirmado` para no romper el alta manual del super-admin), `comprobante_url`, `revisado_por`, `revisado_en`, `motivo_rechazo` (migración `0032_fantastic_malcolm_colcord.sql`). `AdminService.resumenPagos()`/`gananciasPorPeriodo()` ahora sólo cuentan pagos `confirmado`. Nueva cola en `/admin` → Home, "Pagos pendientes de revisión": aprobar/rechazar por pago, con link al comprobante subido. Los helpers de fecha de facturación (`inicioDeMes`/`proximoVencimiento`) se extrajeron de `admin.service.ts` a `common/facturacion.util.ts` para reusarlos desde el módulo self-service sin duplicar la lógica.

### Fix incidental (bloqueaba verificar lo de arriba): `password_changed_at` faltante en 12 de los 18 `test:*-demo`
- La feature de "olvidé mi contraseña" (entrada anterior) sumó `usuarios.password_changed_at`, pero nunca se propagó al DDL inline que cada `test:*-demo` arma a mano — cualquier test que insertara/devolviera un usuario completo fallaba con `column "password_changed_at" of relation "usuarios" does not exist`. Corregido en los 12 archivos que definen `core.usuarios` inline; las 18 suites vuelven a pasar limpio (`plataforma-flow.demo.ts`: 26/26, incluidos los casos nuevos de `estado`/`comprobante_url` de pagos).

**Verificado**: `tsc --noEmit` limpio en backend y web. Las 18 `test:*-demo` pasan (0 fallas). Migración `0032` aplicada contra el `pgdata` real con el procedimiento de siempre (backend parado por PID exacto, conteos de `organizaciones`/`usuarios`/`pagos`/`animales` iguales antes y después, backend reiniciado y confirmado sirviendo el código nuevo — los endpoints nuevos responden 401 sin token en vez de 404). **Sin probar clic a clic en el navegador** — el usuario probó en vivo el rediseño del alta pública (de ahí salieron varios de los ajustes de esta entrada) pero el resto (fix de `/admin`, edición de planes, wizard rediseñado, "Mi plan"/pago con comprobante) quedó pendiente de una pasada visual.

## [2026-09-05] — Recuperar contraseña, calendario nativo en mobile, auditoría de production-readiness

### "Olvidé mi contraseña" (backend + web + mobile)
- Backend: `POST /auth/forgot-password` (email) y `POST /auth/reset-password` (token + password nueva), ambos públicos en `core/auth/`. El primero **siempre** resuelve igual, exista o no la cuenta — no filtra qué emails están registrados. El token es un JWT stateless (mismo patrón que `PortalTokenService` del portal del dueño) con `scope: 'reset_password'` y vencimiento de 30 minutos; como no hay tabla de tokens para marcarlo usado, la invalidación compara el `iat` del token contra una columna nueva `usuarios.password_changed_at` (migración `0031_gorgeous_madripoor.sql`, `ADD COLUMN` nullable — aplicada contra el `pgdata` real con el backend parado por PID exacto, conteos de `usuarios`/`organizaciones` iguales antes y después) — cualquier link emitido antes del último cambio de contraseña deja de servir.
- Nuevo `common/mail/mail.module.ts`/`mail.service.ts`: primer servicio de email del proyecto (no existía ninguno), vía Resend (`resend` SDK, `RESEND_API_KEY`/`MAIL_FROM` en `.env`). Sin API key configurada, el link queda logueado en la consola del backend en vez de enviarse — para no bloquear desarrollo local sin cuenta de Resend creada.
- Web: link "¿Olvidaste tu contraseña?" en `LoginPage.tsx` (nuevo `modo: 'olvide'`, sólo pide el email), y `pages/ResetPasswordPage.tsx` (nueva) que atiende `?resetToken=` (ruteado en `main.tsx`) para elegir la contraseña nueva.
- Mobile: mismo link en `login.tsx`, pero sin formulario de reset propio — como cambiar la contraseña no tiene sentido offline, sólo dispara el pedido (`api.olvidePassword`) y el link del mail se abre en el navegador del celular, que cae en la misma `ResetPasswordPage` de la web.
- **Gap conocido, anotado también en `docs/Plan_Despliegue.md`**: `MAIL_FROM` sigue en el dominio de pruebas de Resend (`onboarding@resend.dev`), que en modo sandbox sólo entrega a la casilla dueña de la cuenta de Resend — hace falta verificar un dominio propio antes de que esto le llegue a un usuario real.

### Calendario nativo para "próxima dosis" (mobile)
- Nuevo `components/CampoFecha.tsx`, con `@react-native-community/datetimepicker` (instalado con `npx expo install`, que corre `npm install` en este monorepo pnpm — se limpió el `package-lock.json` espurio que dejó y se corrió `pnpm install` desde la raíz para que resolviera bien; ver la nota de gotchas nativos en `CLAUDE.md`). Reemplaza el campo de texto libre "AAAA-MM-DD" que tenía la vacunación en `paciente/[id].tsx` — equivalente RN del `<input type="date">` que la web ya usaba para el mismo campo. Requirió un rebuild nativo (`expo run:android`), instalado y verificado en el dispositivo de prueba.

### Auditoría de production-readiness (a pedido del usuario, antes de pasar a una VPS con testeos reales)
No hubo cambios de código en esta parte, sólo relevamiento. `docs/Plan_Despliegue.md` (2026-09-03) ya tenía resuelta la arquitectura de despliegue; se confirmaron gaps puntuales que ese plan no cubría del todo y se agregaron al checklist: CORS totalmente abierto hoy (`main.ts`, `enableCors()` sin restricción de origen — ya estaba anotado, se remarca como el más simple de cerrar), cero rate-limiting en `/auth/login`/`/auth/forgot-password`/`/auth/reset-password` (sin `@nestjs/throttler` ni equivalente en el proyecto), dominio de Resend sin verificar (ver arriba), mobile sin ningún build de release firmado (todo lo hecho hasta ahora es `expo run:android` debug atado al dev-client + Metro corriendo en la PC, no instalable por un tercero — falta EAS Build o un `assembleRelease` con keystore propio, y cambiar el `package` del `app.json`, que sigue en el placeholder `com.anonymous.mobile`), sin `GET /health` ni error tracking (Sentry o similar), y el script de backup existe pero nunca se corrió ni se restauró de prueba.

### Revisión de arquitectura del plan de despliegue: de PM2/bare-metal a Docker
Decisión del usuario, tomada el mismo día: la VPS va a alojar más de un proyecto/ecosistema con el tiempo, así que `docs/Plan_Despliegue.md` se reescribió para correr todo en contenedores en vez de instalado directo en el sistema operativo. Dos decisiones concretas: (1) un único stack de **Caddy + el plugin `caddy-docker-proxy`** para toda la VPS (no uno por proyecto — sólo puede haber un proceso escuchando 80/443 — descubre por *labels* de Docker qué contenedor rutear a qué dominio, sin tocar un `Caddyfile` a mano por cada proyecto nuevo; se prefirió sobre Traefik para no sumar una herramienta nueva ya que Caddy era la elección previa); (2) **Postgres en un contenedor propio por proyecto** (no una instancia compartida con una base por proyecto) — aislamiento total, un upgrade o un backup/restore de un proyecto nunca toca a otro. El backend ahora tiene su propio `Dockerfile` (multi-stage: build con `pnpm`/`nest build`, imagen final con el `node_modules` completo del monorepo — se documentó explícitamente por qué no se recorta a sólo dependencias de prod: `drizzle-kit` es una devDependency pero hace falta dentro del contenedor para `db:migrate`/`db:generate`, y para un solo operador una imagen más pesada es preferible a depurar un recorte de dependencias roto). El script de backup pasa a correr `pg_dump` con `docker compose exec` contra el contenedor, no contra un Postgres del sistema. El resto del plan (recursos de VPS, checklist de endurecimiento, backups lógicos vs. snapshots, DonWeb como proveedor) no cambió, sólo el "cómo" se corre cada pieza.

**Verificado**: `tsc --noEmit` limpio en backend y mobile, `pnpm --filter web build` limpio. Migración `0031` aplicada con verificación de conteos antes/después. Probado en el dispositivo real: link de recuperación logueado en consola (sin `RESEND_API_KEY` configurada al momento de probar), formulario de nueva contraseña, y el datepicker nativo de "próxima dosis". **Sin verificar clic a clic la pantalla de reset en un email real entregado** — pendiente de la verificación de dominio en Resend.

## [2026-09-04] — Analítica de uso, catálogos de referencia (SENASA/vacunas/diagnósticos), landing page pública

### Analítica de uso (a pedido del usuario: "conocer qué pantallas y acciones usan más los clientes")
- Backend: nuevo módulo `analitica/`, tabla `plataforma.eventos_uso` (`tipo`: `'pantalla'`|`'accion'`, `organizacionId`, `usuarioId` nullable, `nombre` free text, `createdAt` — migración aplicada contra el `pgdata` real con el procedimiento de siempre). `POST /analitica/eventos` es un insert liso, cualquier rol autenticado. `AdminService.resumenAnalitica()` agrega top pantallas/acciones/organizaciones por rango de fechas (mismo patrón `gte`/`lte` que `CajasService.estadisticas()`), consumido por la pestaña nueva "Analítica" de `AdminPage.tsx`.
- Web: `api.registrarEvento(sesion, tipo, nombre)` (fire-and-forget) instrumentado en prácticamente todas las acciones de escritura de Huella y, al final de esta entrega, las **18** de `TroperaPage.tsx` (una por formulario/mutación — establecimientos, existencias, movimientos, eventos, animales, evaluaciones andrológicas, muestras, plantillas de tareas, protocolos IATF, tareas). Las pantallas se instrumentan una sola vez, centralizado, en un `useEffect` de `App.tsx` sobre el nombre de la `Vista` actual.
- Mobile: mismo mecanismo — `api.registrarEvento` nuevo en `api/client.ts`, tracking de pantallas centralizado en `app/_layout.tsx` (`useSegments()`, filtrando los segmentos de route-group y el redirect-shim de `(app)/index.tsx`), y acciones instrumentadas en Home, Turnos (atender/crear), la ficha (consulta/vacunación/foto), y Farmacia (venta/ingreso/alta de producto, esta última en el helper compartido `db/altaProducto.ts` para no duplicarla en los dos call sites que lo usan).

### Catálogos de referencia: vademécum SENASA, vacunas y diagnósticos comunes
- Tres tablas globales nuevas (no por organización, mismo espíritu que `core.especies`): `farmacia.vademecum_senasa` (7003 filas, seedeadas desde un CSV real de SENASA que aportó el usuario — sin constraint de unicidad en `certificado`, hay ~245 duplicados legítimos en la fuente), `hce.catalogo_vacunas` y `hce.catalogo_diagnosticos` (41 y 75 filas, seedeadas desde JSON armados por el usuario como sugerencias básicas por especie). Nuevos endpoints de sólo lectura: `GET /farmacia/vademecum-senasa?buscar=`, `GET /hce/catalogo-vacunas?especieId=`, `GET /hce/catalogo-diagnosticos?especieId=`.
- **Corrección de rumbo en el camino** (feedback explícito del usuario tras ver el primer resultado): la primera versión mostraba estos catálogos como un `<select>`/lista cerrada de opciones. Se rehizo como campos de **búsqueda con sugerencias** (`BuscadorSenasa.tsx`, `BuscadorCatalogoVacunas.tsx`, `BuscadorCatalogoDiagnosticos.tsx`, en web y mobile) sobre un campo de texto libre — el catálogo es una guía para autocompletar, nunca una restricción a lo que ya existe ahí. Segunda corrección, más chica: las sugerencias no deben aparecer hasta que el usuario tipeó algo (la primera versión mostraba el catálogo completo con el campo vacío/en foco, ocupando espacio de pantalla sin necesidad).
- Para mobile, esta misma sesión sumó "＋ no aparece: crear producto nuevo" como opción secundaria dentro de los buscadores de producto de Venta rápida e Ingreso de stock (`db/altaProducto.ts`), y un `SelectorDueno.tsx` que vuelve obligatorio elegir o crear un dueño (antes era opcional y una lista plana de todas las personas, que no escalaba).

### Landing page pública
- Nueva `pages/LandingPage.tsx`: página de marketing para campañas, mostrada cuando no hay sesión y la ruta no es `/login` (`App.tsx`). Hero, sección de 3 pasos, features en tabs (contenido tomado de `docs/Comercial_Huella.md`), y una sección de precios armada dinámicamente desde `listarPlanesPublicos()` — cada plan es un CTA a `/login?plan=<id>`, que `LoginPage.tsx` lee al montar para arrancar directo en modo registro con ese plan preseleccionado.

### Otros pendientes chicos de UX resueltos en el camino
- Al finalizar una consulta (web y mobile), el monto cobrado ahora se pide con un default en cero en vez de quedar vacío.
- Mobile: campos de la consulta colapsables (`components/CampoColapsable.tsx`, indicador de lleno/vacío) para que el formulario largo no intimide de entrada; el botón "Guardar consulta" tenía muy poco margen inferior, corregido con padding en el scroll.
- Mobile, pestaña Turnos: filtro simple Hoy/Próximos/Todos, y botón "Atender" que marca el turno y abre la ficha en la sección de consulta (mismo flujo que ya tenía la web).

**Verificado**: `tsc --noEmit` limpio en backend y mobile, `pnpm --filter web build` limpio. Migraciones de los 3 catálogos + `eventos_uso` aplicadas contra el `pgdata` real con el procedimiento de siempre (parar por PID, aplicar, verificar conteos, reiniciar). Probado en el dispositivo Android real conectado (rebuild nativo por las nuevas dependencias — `expo-image-picker`, subida de foto vía `expo-file-system` en vez de `fetch`+`FormData`, que el runtime propio de Expo SDK 57 no soporta para el `{uri,name,type}` clásico de RN). **Sin verificar clic a clic la landing page contra un dominio real** — hoy sólo probada en local.

## [2026-09-03] — Turnos con rango de fechas, y otro lote de pendientes de Huella (recordatorios, agendas, foto de ficha, caja, ingreso de stock)

Dos entregas del mismo día, después del lote de Fase 5c (ver entradas de más abajo).

### Turnos: rango de fechas en la cabecera
- `TurnosPage.tsx` seguía mostrando siempre un único día. Ahora la cabecera suma un campo "hasta" (oculto por defecto): al completarlo, la vista pasa a modo rango y la tabla muestra los turnos de todos los días entre "desde" y "hasta". Hoy sigue siendo el default — cualquier acción de navegación por día (‹/›, "Hoy", click en el calendario, reprogramar, crear turno) vuelve a modo un-solo-día a través de un helper compartido (`irADia()`), así el rango nunca queda "pegado" después de una acción puntual.

### Recordatorios: descartar + filtro por animal
- `hce.vacunaciones` suma `recordatorio_descartado_en` (timestamptz, nullable — migración `0026_thin_spectrum.sql`, aplicada contra el `pgdata` real con el backend parado, conteos de `vacunaciones`/`consultas`/`organizaciones`/`animales` iguales antes/después). `VacunacionesService.recordatorios()` excluye los descartados; nuevo `PATCH /vacunaciones/:id/descartar-recordatorio`.
- `RecordatoriosPage.tsx`: botón "Descartar" por fila de vacuna, y un selector "Todos los animales" (sólo aparece si hay 2+ animales con recordatorio en la ventana actual) que filtra la lista sin pegarle de nuevo al backend.

### "⚙ Agendas" se mudó a Usuarios
- El botón vivía en `TurnosPage.tsx` (cabecera, junto a "＋ Nuevo turno"). Se movió a `UsuariosPage.tsx` (mismo componente `GestionAgendas`, sin cambios internos) — tiene más sentido ahí porque ya es la pantalla `propietario`/`admin`-only de configuración de la organización, y libera la cabecera de Turnos para el selector de rango de arriba.

### Foto en la ficha del animal (antes sólo la subía el dueño desde el portal)
- Nuevo `POST /animales/:id/foto` (multipart, `propietario`/`admin`/`veterinario`/`recepción`) — reusa el mismo `UpdateAnimalDto.fotoUrl` que ya existía, sin service nuevo. El multer/storage compartido con el portal (antes vivía sólo en `portal.controller.ts`) se extrajo a `common/foto-animal-upload.ts` para no duplicarlo entre las dos superficies (portal del dueño vs. ficha de personal).
- `PacienteDetallePage.tsx`: avatar clickeable junto al nombre (mismo patrón que el portal — `comprimirImagen()` en el cliente, sin ninguna librería de imágenes en el backend). `Animal.fotoUrl` faltaba en `api/types.ts` pese a que el backend ya la devolvía siempre (`.select()` sin proyección) — sólo faltaba en el tipo del frontend.
- Nuevo `pedirArchivo()` en `api/client.ts` (junto a `pedir()`): mismo reintento-ante-401 que el resto del cliente, pero sin forzar `Content-Type` (rompería el boundary de `multipart/form-data`) — primer uso de `FormData` en `client.ts` (el portal ya lo hacía, pero con su propio cliente separado sin sesión de refresh token).

### Caja: auto-cierre de una caja olvidada de un día anterior
- Nuevo `CajasService.normalizarDelDia()`: si la caja abierta de la organización quedó de un día anterior (el propietario se olvidó de cerrarla), se cierra sola, sin arqueo humano (`montoDeclarado` queda sin cargar) y siempre en revisión (`estadoAuditoria: 'pendiente'`) — no bloquea a quien la disparó. Se llama explícitamente antes de `GET /caja/cajas/actual`, de un cobro nuevo y de un egreso nuevo — a propósito no vive dentro del `actual()` de lectura pura, para que un simple GET no tenga efectos secundarios en otros contextos que también lo llaman. Los cobros siguen abriendo una caja nueva sola si hace falta (apertura rápida, ya existía); los egresos siguen sin hacerlo — un egreso no es motivo para abrir la caja del día por sí solo.

### Home: acceso rápido "Ingreso de stock" + impacto en Caja
- Nueva tarjeta en la columna derecha del Home (debajo de las 3 KPI), gateada a `ROLES_FARMACIA` (`propietario`/`admin`/`veterinario`, mismo set que ya usa `FarmaciaPage.tsx`) — abre el mismo flujo "+ Ingresos" de Farmacia en un modal. El formulario (`IngresoForm`, antes vivía sólo dentro de `FarmaciaPage.tsx`) se extrajo a `components/IngresoStockForm.tsx` para reusarlo desde los dos lugares sin duplicar la calculadora de bultos ni la doble carga de precio.
- **El precio de compra al proveedor ahora también genera un egreso** (`precioCompra × cantidad`, con el nombre del producto y la cantidad en el concepto) — tanto desde este acceso rápido como desde el "+ Ingresos" de Farmacia, mismo componente. Es best-effort: si el egreso falla (por ejemplo, un `veterinario` cargando el ingreso — Farmacia permite `veterinario` pero Caja no, los roles no coinciden a propósito, ver `CLAUDE.md`), el movimiento de stock y los precios del producto ya se guardaron igual; se avisa con un error inline en vez de deshacer nada.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, los **18** `test:*-demo` en verde (incluidos los 3 mini-schemas que hubo que sumarles `recordatorio_descartado_en` para no romper por el mismo motivo de siempre — `vacunas`/`sync`/`portal`, ver la lección ya anotada en `CLAUDE.md`). `test:caja-demo` sumó 8 checks nuevos para `normalizarDelDia()` (cierre solo con cálculo correcto, el cobro/egreso del día nuevo no se imputa a la caja vieja, un egreso sigue sin abrir caja sola). Migración `0026` aplicada contra el `pgdata` real (backend parado por PID exacto, reiniciado después con `nest start --watch`). **Sin verificar clic a clic en el navegador todavía** — pendiente una pasada visual de los 5 puntos antes de darlos por completamente cerrados.

## [2026-09-03] — Documentación al día + plan de despliegue

A pedido del usuario: actualización completa de la documentación (que había quedado con drift real, algunos casos declarados desde el 2026-08-30 y nunca corregidos) más un plan de despliegue nuevo de punta a punta.

- **`CLAUDE.md`**: sumados los 4 `test:*-demo` que faltaban en la lista de comandos (`agendas`/`solicitudes`/`usuarios`/`portal`, ya eran 18 en total), toda la sección de arquitectura backend actualizada con `caja/` (que no estaba documentado ahí **pese a existir desde la Fase D**, 2026-08-29 — drift preexistente, no de hoy), y los cambios de la Fase 5c en `admin/`/`solicitudes/`/`portal/`/`core/usuarios/`/`hce/consultas`. Sección de arquitectura web sumó 7 bullets nuevos (wizard, alta self-service, portal, Recordatorios, Caja, solicitud+plan).
- **`docs/Roadmap_Ecosistema.md`**: nueva sección "Fase 5c" resumiendo las 5 entregas + la verificación en navegador del 2026-09-03 (detalle completo queda en este changelog, el Roadmap sólo resume estado). Tabla de estado de avance actualizada (Portal, Consola de administración, dos filas nuevas: Caja/Onboarding). "Próximos pasos" reordenado: producción (VPS) pasa a ser el ítem #1, con nota aclarando que "Home de facturación" (ya cerrado) y "Facturación ARCA" (Fase 5, sigue pendiente) son dos cosas distintas que no hay que confundir.
- **`docs/Estructura_Proyecto.md`**: reescrito el árbol completo de `apps/backend` (sumado `caja/` que faltaba del todo, `admin/planes/`, `common/verificar-limites-roles.ts`, los archivos de schema nuevos) y de `apps/web` (el que tenía la advertencia de drift desde el 2026-08-30 — ahora sí refleja `nav/config.ts`, `HuellaHomeSection.tsx` reemplazando a `DashboardPage.tsx`, `CajaPage.tsx`, y todos los componentes/páginas nuevos del lote de hoy). De paso se encontró y corrigió otro dato viejo: el documento decía que `apps/backend/.env.example` ya existía — **no existía**, se creó recién ahora (ver más abajo).
- **`docs/Protocolo_Pruebas.md`**: fecha de cobertura actualizada, nota sobre la pasada de verificación en navegador de hoy, y corregido un límite conocido que ya no era cierto ("Tropera sin seguimiento individual" — la Fase E lo agregó hace rato, convive con los conteos agregados sin reemplazarlos).
- **`apps/backend/.env.example`** (nuevo — no existía pese a que `Estructura_Proyecto.md` lo daba por hecho): todas las variables de entorno reales del backend, incluidas las dos nuevas de 2026-09-03 (`BACKEND_PUBLIC_URL`, `PORTAL_BASE_URL`).
- **`docs/Plan_Despliegue.md`** (nuevo): arquitectura recomendada para producción (una VPS, dos subdominios `app.`/`api.` — porque `VITE_API_URL`/`PORTAL_BASE_URL`/`BACKEND_PUBLIC_URL` ya asumen orígenes absolutos, no hace falta reverse-proxy con rewrite de rutas), recursos de VPS recomendados (2vCPU/4GB/60-80GB — con nota sobre por qué priorizar una región cercana a Argentina, ej. São Paulo, sobre el precio más bajo de una región europea/EE.UU.), Caddy para HTTPS automático (con el equivalente en Nginx+certbot para quien lo prefiera), PM2 para el proceso del backend, checklist de endurecimiento pre-lanzamiento (CORS hoy abierto a cualquier origen, `JWT_SECRET`/`BACKEND_PUBLIC_URL` a fijar antes de la primera foto real subida), backups (Postgres + `uploads/`) y checklist de deploy paso a paso.

**Verificado**: los 5 markdown tocados tienen los fences de código balanceados (chequeo automático). Sin verificar contra una VPS real todavía — el plan de despliegue es exactamente eso, un plan; F0.1 sigue ⏳ hasta que se ejecute.

## [2026-09-03] — Verificación en el navegador de las 5 entregas del día + 2 bugs reales encontrados y corregidos

Pasada de pruebas real en el navegador (headless Chromium vía Playwright, instalado ad-hoc para esta sesión — no quedó nada de esto en el repo) contra un backend/web levantados aparte en puertos 3001/5174 con una base PGlite descartable (`/tmp/test-pgdata`), para no tocar el `pgdata` real. Se recorrió de punta a punta: alta de plan → solicitud de cuenta con selección de plan → aprobación → primer login del propietario → wizard (alta de usuario + agenda de veterinario + peluquería) → turno atendido desde Home → consulta con costo → primera venta con la caja cerrada → Estadísticas de Caja → envío del link del portal por WhatsApp desde Recordatorios → portal magic-link con subida de foto → portal público por código. Todo lo de las cinco entregas de hoy (ver las cinco entradas de abajo) se comportó como estaba diseñado — **con dos excepciones reales, ambas corregidas en el momento**:

1. **La lista de organizaciones (y Home) no se refrescaba sola después de aprobar una solicitud.** `Solicitudes` (dentro de `AdminPage.tsx`) sólo recargaba su propia bandeja de pendientes al aprobar/rechazar; la organización nueva (con su plan y fecha de activación ya aplicados) quedaba invisible hasta recargar la página entera a mano. Contenía un bug preexistente (no introducido en las entregas de hoy, pero recién visible ahora que la Fase 1 le dio contenido real a la pantalla de Home). Arreglado sumando un callback `onCambio` que además dispara `cargarOrgs()`/`cargarPagos()` del `Panel` padre.
2. **Las fotos subidas desde el portal quedaban guardadas en `dist/uploads/` en vez de `apps/backend/uploads/`.** `main.ts` y `portal.controller.ts` calculaban la carpeta de subida con `join(__dirname, ...)` — pero `nest start --watch` corre el JS ya compilado en `dist/`, así que ahí adentro `__dirname` apunta a `dist/`, no a `src/`. En la práctica esto significaba que cualquier foto subida se perdía en el próximo rebuild (o directamente en el próximo restart del watcher). Arreglado usando `process.cwd()` en cambio — mismo criterio que ya usa `scripts/init-local-db.mjs` para `DATABASE_PATH`, que sí es estable entre dev y prod.

Ninguno de los dos bugs estaba cubierto por los `test:*-demo` (llaman a los services directo, nunca levantan la app HTTP completa ni el flujo de dos componentes React coordinándose) — es exactamente el tipo de problema que sólo aparece corriendo la app real. Vale la pena tenerlo en cuenta: la suite de demos da confianza en la lógica de negocio, no reemplaza una pasada real en el navegador antes de dar builds por buenos.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, los 18 `test:*-demo` en verde, jest sin regresiones — todo corrido de nuevo después de los dos fixes. Los servidores de prueba se bajaron por PID exacto al terminar (`lsof -ti :3001/:5174 | xargs kill`), sin tocar los puertos reales (3000/5173, libres antes y después). El `pgdata` real de la organización no se tocó en ningún momento.

## [2026-09-03] — Huella operativo: botón "Atender" en Home, apertura rápida de caja, costo en consultas, estadísticas en Caja

Quinta y última fase del lote de pendientes de negocio (ver las cuatro entradas de abajo).

- **Botón "Atender" en "Turnos de hoy" (Home)**: la tabla ya excluía los turnos cancelados/atendidos, pero cada fila sólo abría la ficha del paciente sin tocar el estado. Ahora, para `veterinario`/`propietario`, los turnos `confirmado`/`reprogramado` tienen un botón "Atender" que marca el turno como atendido (`PATCH /turnos/:id/estado`, ya existía) y abre la ficha con "Nueva consulta" ya desplegada — mismo flujo que ya tenía `TurnosPage.tsx`, reproducido acá.
- **Apertura rápida de caja**: en vez de "primera atención" (que en la mayoría de los casos la hace un veterinario, que no tiene permiso para abrir caja — `abrir()` es `propietario`/`admin`/`recepción`), el gancho real quedó en el **primer cobro/venta del día**: `CobrosService.crear()` ya no rechaza si no hay caja abierta, la abre sola (monto inicial 0) y sigue. Esto cubre tanto "Venta común" del Home (que antes bloqueaba con "Ir a Caja") como el cobro manual de la pestaña Caja — a `VentaRapidaModal.tsx` se le sacó por completo el pre-chequeo de "¿hay caja abierta?" (y el prop `onIrACaja`, que quedó sin uso). Los egresos **no** cambiaron: siguen exigiendo una caja ya abierta a propósito (un egreso no es un buen motivo para abrir la caja del día sola).
- **Costo en consultas**: `hce.consultas` suma `costo` (numeric, nullable en la DB, **obligatorio** en `CreateConsultaDto` desde ahora — el 0 es válido, para cortesías). No hay un estado formal de "cerrar" una consulta (nunca lo hubo), así que el costo se pide en el único submit que existe ("Guardar consulta"/"Guardar cambios"). A propósito **no** genera un cobro automático en Caja — mismo criterio de acoplamiento flojo que la dispensa F4.3: el costo queda como dato de referencia en la ficha, el cobro real (con método de pago, etc.) se sigue cargando a mano.
- **Estadísticas en Caja**: nueva pestaña "Estadísticas" (`GET /caja/cajas/estadisticas?desde&hasta`, sólo propietario/admin) — totales de cobros/egresos/neto del período, cantidad de cajas, desglose por método de pago y por día. Todo calculado en JS sobre las filas del rango (no hace falta agregación SQL para el volumen de una caja chica).

**Encontré y corregí una regresión real en el camino**: al agregar `fechaActivacion` a `organizaciones` en la primera entrega de este lote (ver más abajo), rompí silenciosamente los **otros 12** `test:*-demo` preexistentes (`auth`, `animales`, `personas`, `vacunas`, `turnos`, `sync`, `roles`, `macros`, `indicaciones`, `tropera`, `farmacia`, `agendas`) — cada uno tiene su propio mini-schema de Postgres que había quedado desactualizado, y `jest` (lo único que corría en el chequeo de rutina) no los ejecuta, así que venían fallando sin que ningún chequeo lo mostrara. Se detectó recién ahora al correr la suite completa de `test:*-demo` por primera vez en toda la sesión. Corregidos los 12 (sumando `fecha_activacion` a su `core.organizaciones`, y `costo` a `hce.consultas` en los 3 que también la declaran). **Lección para la próxima vez que se toque una columna de `organizaciones` o `consultas`**: correr los 18 `test:*-demo` completos, no sólo el del módulo que se está tocando — exactamente lo que ya había pasado antes con `farmacia`/`indicaciones`/`turnos`/`sync` (ver entradas de agosto en este changelog) y volvió a pasar acá.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, jest sin regresiones. Los **18** `test:*-demo` corren limpios (incluida la corrección de la regresión). `caja-flow.demo.ts` y `hce-flow.demo.ts` se reescribieron para llamar a las clases de servicio reales (`CajasService`/`CobrosService`/`EgresosService`/`ConsultasService`) en vez de una reimplementación de su lógica — el estilo que tenían antes es exactamente lo que dejó pasar que `test:caja-demo` siguiera "verde" probando un cobro que rechazaba sin caja abierta, un comportamiento que ya no es real. **Sin verificar clic a clic en el navegador**: falta confirmar visualmente el botón "Atender", que Venta Rápida ya no pida caja abierta, el campo de costo en el formulario de consulta y la pestaña Estadísticas de Caja.

## [2026-09-03] — Portal del dueño: estilos unificados con el resto de la app, envío del link desde Recordatorios, foto de perfil de la mascota

Cuarta fase del mismo lote de pendientes (ver las entradas de abajo). Cubre las tres mejoras pedidas del portal — sin tocar la lógica de negocio existente (turnos/vacunas/historia clínica siguen igual).

- **Estilos unificados**: `PortalDuenoPage.tsx` (público por código) y `PortalAccesoPage.tsx` (magic-link) tenían cada una su propio bloque `<style>` con colores hardcodeados (`#0E7C6B`, etc.), distintos de los tokens reales de la app. Ambas usaban clases casi idénticas (`pd-*`) duplicadas entre los dos archivos. Se unificó en `pages/portalEstilos.ts` (nuevo, un solo `CSS_PORTAL` importado por las dos), ahora con los tokens reales (`var(--verde)`, `var(--border)`, `var(--card)`, etc. — ya estaban disponibles: `main.tsx` carga `styles.css` globalmente para toda la SPA, portal incluido, sólo que estas dos páginas no lo estaban aprovechando). El layout mobile-first de una sola columna se mantuvo igual (no tiene equivalente en el shell `.app`/`.layout-2col` de la app interna).
- **Envío del link de acceso desde Recordatorios**: `RecordatoriosPage.tsx` ya resolvía el dueño de cada fila (vacunas y turnos) pero no ofrecía compartir el portal. Nuevo botón "Enviar portal" por fila: genera el acceso (`POST /portal/acceso/:personaId`, ya existía, sólo le faltaba un caller acá) y, con teléfono, abre WhatsApp con el link precargado (mismo patrón `wa.me` que ya se usaba para el aviso de vacuna); sin teléfono, lo copia al portapapeles. La ventana de WhatsApp se abre *antes* del `await` (con `about:blank`, se completa la URL después) porque abrirla después de esperar una respuesta async suele quedar bloqueada por el popup blocker al dejar de contar como gesto directo del click.
- **Foto de perfil del animal (gap real: no existía ninguna infraestructura de upload)**: no había `multer`/`FileInterceptor` ni storage de archivos en todo el backend — se agregó desde cero. Nuevo `POST /portal/animales/:animalId/foto` (multipart, sólo magic-link — el portal público por código quedó fuera de este alcance a propósito, ver nota abajo), guardado en filesystem local (`apps/backend/uploads/`, gitignoreado, sin servicio de storage externo configurado) y servido como estático (`app.useStaticAssets`, `main.ts`). Verifica propiedad (la mascota tiene que ser de quien está autenticado) antes de tocar nada, mismo criterio que `solicitarTurno()`. La compresión pedida ("buena calidad pero lo más liviana posible") se resuelve **100% en el cliente** (`utils/comprimirImagen.ts`, canvas: resize a 800px de lado más largo + recodifica JPEG a calidad 0.75) — no hay ninguna librería de imágenes en el backend (ni sharp ni similar), así que no tenía sentido mandar el archivo pesado para comprimirlo del otro lado. `animales.fotoUrl` ya existía en el schema pero **no se usaba en ningún lado del frontend** hasta ahora (sólo lo leía el PDF del carnet, casi siempre null); ambas páginas del portal ahora la muestran si existe.
  - Decisión de alcance: el upload sólo se agregó al portal **magic-link** (`X-Portal-Token`, identidad real del dueño), no al portal público por código (sin ningún token, el código impreso en el carnet/QR *es* la credencial — permitir ahí una escritura, no sólo lectura, hubiera bajado la barra de seguridad de esa superficie).

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, jest sin regresiones. Nuevo `test:portal-demo` (11/11 OK) — primer test de este módulo, cubre `resumen()` (incluida `fotoUrl`, aislado por organización), `solicitarTurno()` y `actualizarFoto()` (ambos rechazan con 403 una mascota que no es del dueño autenticado). `test:plataforma-demo` (26/26), `test:usuarios-demo` (14/14) y `test:solicitudes-demo` (17/17) sin regresiones. **Sin verificar clic a clic en el navegador**: falta confirmar visualmente el nuevo look del portal, el botón "Enviar portal" (incluido el caso sin popup blocker) y la subida/compresión de una foto real.

## [2026-09-03] — Wizard de configuración rápida: alta de usuarios self-service + agendas de veterinarios y peluquería

Tercera fase del mismo lote de pendientes (ver las dos entradas de abajo): al primer login del propietario de una organización recién aprobada, un wizard de pantalla completa lo guía para dar de alta a su equipo y armar las primeras agendas.

- **Alta de usuarios self-service (gap real que no existía)**: hasta ahora sólo el super-admin de plataforma podía agregar miembros a una organización (`POST /admin/organizaciones/:id/miembros`). Nuevo `POST /usuarios` (tenant-scoped, `propietario`/`admin`, `UsuariosService.agregarMiembro()`) — misma lógica que el de `/admin` (reutiliza el usuario si el email ya existe en la plataforma, respeta el cupo por rol del plan), pero llamable por la propia organización sin pasar por `/admin`. Nuevo `GET /usuarios/limites-plan` expone cupo/usados por rol (antes esto sólo se calculaba puertas adentro de `AdminService`, invisible para la organización).
- `admin/verificar-limites-roles.ts` se movió a `common/verificar-limites-roles.ts` — ahora lo usan tanto `admin` como `core/usuarios`, y que `core` dependiera de `admin` era la dirección de dependencia equivocada.
- **Web**: `components/FormAltaMiembro.tsx` (nuevo, reusable) — formulario de alta con checkboxes de rol que se deshabilitan solos si el plan ya agotó el cupo de ese rol. Se usa en dos lugares: `UsuariosPage.tsx` suma un botón "+ Nuevo usuario" permanente (antes esa pantalla sólo reseteaba contraseñas), y el primer paso del wizard.
- **`components/WizardConfiguracionRapida.tsx`** (nuevo): se dispara una sola vez por usuario (mismo patrón de flag en `localStorage` que `TutorialGuiado`, pero acá **bloquea** el resto de la app mientras está activo — es un setup guiado, no una ayuda superpuesta) para quien tiene el rol `propietario` (quien siempre queda a cargo de una organización recién aprobada — ver la entrada de "Solicitud de cuenta" más abajo). Pasos: alta de usuarios → agenda por veterinario (reutiliza `POST /agendas` + `POST /agendas/:id/bloques`, ya existentes — no hubo que tocar el backend de agendas) → agenda de peluquería canina (misma mecánica, agenda "sin profesional") → fin. Cada paso se puede saltar; el modelo de horario es simplificado a propósito (un solo rango horario para todos los días elegidos) — para horarios más finos, `TurnosPage.tsx` → "⚙ Agendas" sigue disponible como siempre.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, jest sin regresiones. Nuevo `test:usuarios-demo` (14/14 OK): cupo por rol (incluido el caso límite "límite en 0 rechaza, no es lo mismo que sin límite"), reutilización de un usuario que ya existe en la plataforma, rechazo de membresía duplicada, aislamiento por organización. `test:plataforma-demo` (26/26) y `test:solicitudes-demo` (17/17) sin regresiones tras mover `verificar-limites-roles.ts`. **Sin verificar clic a clic en el navegador**: falta confirmar visualmente el wizard completo (los 5 pasos) y el nuevo botón "+ Nuevo usuario" de Usuarios.

## [2026-09-03] — Solicitud de cuenta: selección de plan obligatoria, sólo "cuenta nueva", datos filiatorios completos e info de roles

Segunda fase del mismo lote de pendientes de negocio (ver la entrada de Home de facturación más abajo): el form público de alta ahora obliga a elegir un plan y a cargar los datos de quien pide la cuenta completos.

- **Form simplificado**: se sacó de la UI la opción "unirme a una veterinaria existente" — `LoginPage.tsx` ya sólo ofrece "cuenta nueva". El backend sigue aceptando `tipo: 'unirse'` a nivel de datos/aprobación (por si queda algo pendiente de antes), pero `CrearSolicitudDto` ya sólo valida `tipo: 'crear'` — no se puede enviar una solicitud "unirse" nueva.
- **Datos filiatorios obligatorios**: `telefono` y `dni` pasan de opcionales a requeridos en `CrearSolicitudDto` (columnas sin cambios, ya existían).
- **Selección de plan**: `core.solicitudes` suma `plan_id` (migración `0024_gorgeous_namorita.sql`, aplicada contra el `pgdata` real, mismo protocolo de siempre). Nuevo endpoint público `GET /solicitudes/planes` (sólo devuelve planes `activo: true`, reutilizando el flag de la entrega anterior). `SolicitudesService.crear()` rechaza con 404 un `planId` inexistente y con 400 uno deshabilitado para altas nuevas. Al aprobar, el plan elegido y `fechaActivacion: now()` se trasladan directo a la organización nueva — ya no hace falta cargarlos a mano desde /admin después de cada alta.
- **Web**: el form muestra, al elegir un plan, cuántos usuarios de cada rol incluye. Nuevo catálogo `config/rolesInfo.ts` + componente `<InfoRoles />` (bloque colapsable "¿Qué puede hacer cada rol?") — se usa tanto en el form público como en la pestaña Planes de /admin, para que el super-admin también tenga a mano qué desbloquea cada rol.
- `AdminPage.tsx`: la tarjeta de cada solicitud pendiente ahora muestra el plan solicitado.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio. Nuevo `test:solicitudes-demo` (17/17 OK): plan inexistente/deshabilitado rechazan la solicitud, `planesDisponibles()` filtra correctamente, aprobar traslada plan + fecha de activación a la organización y crea la membresía "propietario", email duplicado/solicitud ya resuelta rechazan como corresponde. `test:plataforma-demo` sigue en 26/26 (sin regresiones). **Sin verificar clic a clic en el navegador**: falta confirmar visualmente el nuevo selector de plan y el bloque de roles en el form de alta.

## [2026-09-03] — Admin: Home de facturación (pagos por organización, fecha de activación, ganancias acumuladas)

Primera fase de un lote de pendientes de negocio en `/admin` (control de pagos, onboarding, portal, mejoras de Huella): esta entrega cubre sólo la base de facturación — las demás fases (solicitud de cuenta con selección de plan, wizard de configuración rápida, portal, Huella operativo) quedan para iteraciones siguientes.

- **Schema**: `core.organizaciones` suma `fecha_activacion` (timestamp, nullable — día del mes que define cuándo se factura; si está vacío se usa `createdAt`). Nueva tabla `plataforma.pagos` (organización, período mensual, monto, medio de pago, observaciones, quién lo cargó) — registro manual, sin integración con ninguna pasarela. Migración `0023_heavy_silverclaw.sql`, aplicada contra el `pgdata` real (backend parado, sin nada escuchando en :3000; conteos de `organizaciones`/`planes`/`usuarios` iguales antes/después, `pagos` en 0 filas nuevas).
- **`AdminService`**: `setAcceso` ahora acepta `fechaActivacion` y **rechaza asignar un plan con `activo: false`** a una organización que no lo tenía ya (el flag `planes.activo` existía en el schema pero no lo hacía cumplir nadie todavía). Nuevo `registrarPago`/`listarPagos`/`resumenPagos` (próximo vencimiento + "¿pagó este mes?" por organización) y `gananciasPorPeriodo` (suma por mes calendario + total histórico). El cálculo de "próximo vencimiento" compara sólo fechas (no hora) para que, si hoy es justo el día de facturación, muestre "vence hoy" en vez de saltar al mes que viene.
- **Web (`AdminPage.tsx`)**: nueva pestaña "Home" (ahora la pantalla de entrada del panel) con la tabla de pago por organización + "Registrar pago" (modal con historial) y la tarjeta de ganancias acumuladas por mes. La tarjeta "Acceso" de cada organización suma el campo "Fecha de activación". En "Planes", el toggle antes genérico "Activar/Desactivar" ahora dice explícitamente "Habilitar/Deshabilitar para altas nuevas".
- Reutiliza `planes.activo` (ya existía, no se agregó una columna nueva) en vez de duplicar el concepto — sólo le faltaba que algo lo hiciera cumplir.

**Verificado**: `tsc --noEmit` limpio en backend y web, `pnpm --filter web build` limpio, `pnpm --filter backend test` (suite jest preexistente) sin regresiones. `test:plataforma-demo` (ampliada con los casos nuevos: rechazo de plan deshabilitado, fecha de activación → próximo vencimiento incluyendo el caso límite "vence hoy", pagos → "pagó este mes", historial, ganancias por período) — 26/26 OK. **Sin verificar clic a clic en el navegador** (sesión sin herramienta de automatización de browser): falta confirmar visualmente la pantalla Home y el modal de registrar pago.
